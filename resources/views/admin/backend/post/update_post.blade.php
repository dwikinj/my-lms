...
<div class="page-content">
    ...
    <div class="card">
        <div class="card-body p-4">
            ...
            <form method="POST" id="myForm" enctype="multipart/form-data" action="{{ route('blog.post.update') }}"
                class="row g-3">
                @csrf
                @method('PUT') 
                        ...
                        <div class="mb-3">
                            ...
                            <select name="blog_category_id" id="blog_category_id" class="form-select mb-3" aria-label="Default select example">
                                ...
                                @foreach ($blogCategory as $category)
                                    <option value="{{ $category->id }}" {{ $post->blog_category_id == $category->id ? 'selected' : '' }}>
                                        {{ $category->category_name }}
                                    </option>
                                @endforeach
                            </select>
                            <span class="invalid-feedback" id="blog_category_id_error"></span> {{-- Tempat untuk error backend --}}
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <div class="mb-3">
                            ...
                            <input class="form-control" name="post_title" type="text" id="post_title" value="{{ $post->post_title }}">
                            <span class="invalid-feedback" id="post_title_error"></span> {{-- Tempat untuk error backend --}}
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <div class="mb-3">
                            ...
                            <input type="text" name="post_tags" id="post_tags" class="form-control" data-role="tagsinput" value="{{ $post->post_tags }}">
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-12 mt-1">
                    <img id="imagePreview" src="{{ !empty($post->post_image) ? url($post->post_image) : url('upload/no_image.jpg') }}" alt="Post Preview" class="rounded p-1 bg-primary"
...
$(document).ready(function() {
    ...
    $('#myForm').on('submit', function(e) {
        ...
        $.ajax({
            url: actionUrl,
            method: 'POST', // HTML form dikirim sebagai POST, Laravel akan menghandle PUT dari _method
            data: formData,
